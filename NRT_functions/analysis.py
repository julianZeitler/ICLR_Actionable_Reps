...
def quantitative_analysis_seq(g0, S, om, parameters, counter, res=70, savepath=None):
    ...
    maps_very_small = [V_very_small[i,:] for i in range(V_very_small.shape[0])]
    ...
    fig_score, axes = plt.subplots(1, 3, figsize=(18, 5))

    score_60, score_90, max_60_mask, max_90_mask, sac, max_60_ind = zip(
      *[scorer.get_scores(rm.reshape(res, res)) for rm in tqdm(maps_very_small)])

    axes[0].hist(score_60, range=(-1,2.5), bins=15)
    axes[0].set_title('Very Small Ratemaps')
    axes[0].set_xlabel('Grid score')
    axes[0].set_ylabel('Count')

    # Add max and mean statistics for small ratemaps
    max_score_very_small = np.max(score_60)
    mean_score_very_small = np.mean(score_60)
    axes[0].text(0.05, 0.95, f'Max: {max_score_very_small:.3f}\nMean: {mean_score_very_small:.3f}',
    ...
    axes[1].set_title('Small Ratemaps')
    ...
    axes[1].text(0.05, 0.95, f'Max: {max_score_small:.3f}\nMean: {mean_score_small:.3f}',
                 transform=axes[0].transAxes, verticalalignment='top',
                 bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

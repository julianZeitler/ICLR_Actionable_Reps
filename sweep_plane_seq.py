...
base_params = {
    "seq_len": 150, "batch": 0, "D": 65, "T": 5000, "K": 5, "N_rand": 150, "phi_std": 1,
    "N_shift": 15, "resample_iters": 5, "save_iters": 1, "print_iters": 1000,
...
results = run_seq_parameter_sweep(
    ...
    base_savepath=f"data/{datetime.strftime(datetime.now(), '%y%m%d')}/constant_n_per_step_1000_varied_data",
    key_seed=0,
    generate_plots=True,
    same_init_across_K=True,  # Set to True to use same initialization for all K runs
    use_dataset_variants=True  # Set to True to use different dataset variants for each K run
)